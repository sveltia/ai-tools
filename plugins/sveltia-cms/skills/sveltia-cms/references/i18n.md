...
When an entry is moved to a different folder, each locale’s file goes below the localized folder chain, and its child entries are moved along in every locale. Renaming the folder is done with the [Slug Editor](https://sveltiacms.app/en/docs/ui/content-editor#slug-editor), which offers a folder name per locale when the slugs are localized.
