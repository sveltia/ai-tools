...
To use i18n support in your Sveltia CMS project, you need to define the `i18n` option in the top-level, collection-level, and field-level configurations. For a [file collection](https://sveltiacms.app/en/docs/collections/files), file-level configuration is also required. See [Configuration Options](https://sveltiacms.app/en/docs/i18n/options) for what each level accepts.
...
- [Configuration Options](https://sveltiacms.app/en/docs/i18n/options): The `i18n` option at the top, collection, file and field levels, and which locales are enabled by default when creating an entry.
- [Content Structures](https://sveltiacms.app/en/docs/i18n/structures): How localized content is stored on disk for entry and file collections, including nested collections and page bundles.
- [Localized Slugs and Preview Paths](https://sveltiacms.app/en/docs/i18n/slugs): Localize entry slugs, make them editable, use UUIDs for non-Latin content, and include the locale in preview URLs.
...
- The [`summary` entry collection option](https://sveltiacms.app/en/docs/collections/entries/listings#summaries) supports the `{{locales}}` template tag to show enabled entry locales in the entry list.
...
The `i18n` option can be defined at the top level of your configuration, on a collection, on a file in a [file collection](https://sveltiacms.app/en/docs/collections/files), and on individual fields. This page explains what each level accepts. For a complete example that sets up all levels at once, see the [Internationalization overview](https://sveltiacms.app/en/docs/i18n#configuration).
...
- `structure`: Defines how localized content is stored. See [Content Structures](https://sveltiacms.app/en/docs/i18n/structures) for the available options. An entry collection can instead put the `{{locale}}` placeholder in its `folder` option to say where the locale folder goes, as in `content/{{locale}}/posts`. See [Custom Locale Folder Placement](https://sveltiacms.app/en/docs/i18n/structures#custom-locale-folder-placement).
...
Note that the `structure` option at the collection level only applies to [entry collections](https://sveltiacms.app/en/docs/collections/entries). For [file collections](https://sveltiacms.app/en/docs/collections/files), see [File Collections](https://sveltiacms.app/en/docs/i18n/structures#file-collections) on the Content Structures page.
...
Note that the `structure` option does not apply to [file collections](https://sveltiacms.app/en/docs/collections/files). See [File Collections](https://sveltiacms.app/en/docs/i18n/structures#file-collections) on the Content Structures page for more details.
...
- The [KeyValue field type](https://sveltiacms.app/en/docs/fields/keyvalue) additionally accepts `duplicate_keys`, which makes the keys duplicate — they are copied from the default locale and read-only in non-default locales — while the values can be edited separately for each locale.
...
Source: https://sveltiacms.app/en/docs/i18n/options

---

## I18n Content Structures

The `structure` i18n option defines how localized content is stored on disk. It is set in the [top-level or collection-level configuration](https://sveltiacms.app/en/docs/i18n/options). Sveltia CMS supports five different structures:
...
An entry collection can also put the `{{locale}}` placeholder in its `folder` option to place the locale folder anywhere in the path, as in `content/{{locale}}/posts`. See [Custom Locale Folder Placement](#custom-locale-folder-placement) below.
...
#### Custom Locale Folder Placement

The `multiple_folders` structure puts the locale folder right below the collection folder, and `multiple_root_folders` puts it in front of everything. Some sites keep the locale folders somewhere in between: Hugo’s [translation by content directory](https://gohugo.io/content-management/multilingual/#translation-by-content-directory), for example, gives each language a `contentDir` like `content/en` and `content/de`, so the locale sits below the `content` folder but above the sections in it. To support such a layout, put the `{{locale}}` placeholder in the collection’s `folder` option where the locale folder goes:
...
The placeholder is filled in with each locale code, and the `path` and file name follow as usual:
...
└─ content/
   ...
   ├─ en/
   │  └─ posts/
   │     └─ hello.md  # English (default locale)
   └─ fr/
      └─ posts/
         └─ hello.md  # French
```

The placeholder takes precedence over the `structure` option for that collection: the collection gets a folder per locale wherever the placeholder is, whatever the site-level structure says, so the other collections can keep a different structure. Everything that works with the `multiple_folders` structure works here too, including [localized slugs](https://sveltiacms.app/en/docs/i18n/slugs#localizing-entry-slugs), [nested collections](#nested-collections) and [page bundles](#page-bundles). The collection’s [special index file](https://sveltiacms.app/en/docs/collections/entries/listings#managing-hugo-s-special-index-file) lands in each locale’s collection folder, e.g. `content/en/posts/_index.md`, which is where Hugo expects a section’s list page to be. Note that the `multiple_folders` and `multiple_root_folders` structures are the same as `folder: content/posts/{{locale}}` and `folder: {{locale}}/content/posts`, respectively.

A few rules apply to the placeholder:

- It must be a whole folder name, used once, like `content/{{locale}}/posts`, `{{locale}}/posts` or `content/{{locale}}`. A folder name that merely contains it, like `content-{{locale}}`, is rejected.
- The collection must have i18n enabled, with locales defined at the site level or on the collection. Otherwise the placeholder has nothing to be filled in with, and the configuration is rejected.
- It’s the `folder` option that takes the placeholder, not the `path` option, which is relative to the folder and filled in per entry.

**Omitting Default Locale from File Path**

When the `omit_default_locale_from_file_path` option is set to `true`, the placeholder folder is left out of the default locale’s path:
...
For example, with `en` as the default locale, the English file would be located at `content/posts/hello.md`, while the German and French files would be located in their respective locale folders:
...
# folder: content/{{locale}}/pages
content/en/pages/products/hardware/_index.md
content/de/pages/products/hardware/_index.md
```
...
# folder: content/{{locale}}/pages
content/en/pages/products/hardware.md
content/de/pages/products/hardware.md
```
...
- Moving an entry with the [path editor](https://sveltiacms.app/en/docs/collections/entries/nested#choosing-a-parent-folder) moves every locale’s file in the same commit. In the `subfolders` mode it takes the entry’s child entries along, in every locale.
- [Entry-relative media](https://sveltiacms.app/en/docs/media/internal#using-entry-relative-folders) is stored once, not once per locale. With the `multiple_folders` and `multiple_root_folders` structures, or a `folder` with the `{{locale}}` placeholder, where each locale has a folder of its own, the file is saved in the default locale’s folder and every locale’s entry refers to it by the same relative path. The other structures keep each locale’s file in the folder holding the entry, so the media already sits beside all of them.

##### Localized Folder Names

The folder names can be localized as well, giving nested content fully localized permalinks. In the `subfolders` mode, a folder is named after the slug of the entry stored in it, so [localizing the entry slugs](https://sveltiacms.app/en/docs/i18n/slugs#localizing-entry-slugs) with the `localize` filter localizes the whole folder chain above an entry along with the entry itself. With the `multiple_folders` structure, a `products/hardware` entry in English could then be stored as follows:
...
A [page bundle](https://gohugo.io/content-management/page-bundles/) keeps an entry’s content file and its media together in one folder. Give a collection a [`path`](https://sveltiacms.app/en/docs/collections/entries/slugs#using-subfolders) option ending in a fixed file name to store each entry as a leaf bundle, or use the `subfolders` mode of a [nested collection](https://sveltiacms.app/en/docs/collections/entries/nested#nesting-page-bundles) to store each one as a branch bundle. Combined with [entry-relative media](https://sveltiacms.app/en/docs/media/internal#using-entry-relative-folders), uploads land in the bundle instead of a shared media folder.
...
The `multiple_folders` and `multiple_root_folders` structures, as well as a `folder` with the [`{{locale}}` placeholder](#custom-locale-folder-placement), give each locale a bundle of its own. The media is not copied into each one: it’s stored in the default locale’s bundle, and every locale’s entry refers to it by the same relative path:
...
# folder: content/{{locale}}/posts
content/en/posts/my-first-post/index.md   # cover: image1.jpg
content/de/posts/my-first-post/index.md   # cover: image1.jpg
content/en/posts/my-first-post/image1.jpg
```

Sveltia CMS resolves that path across locales, so the image appears in the content editor whichever locale you are editing. Hugo resolves it as well, because [a page bundle inherits the resources of its translated pages](https://gohugo.io/content-management/multilingual/) — but only where the two are linked as translations, which Hugo determines from each language’s `contentDir`. The last layout above is Hugo’s own [translation by content directory](https://gohugo.io/content-management/multilingual/#translation-by-content-directory), with `contentDir` set to `content/en` and `content/de`. If your framework has no equivalent mechanism, prefer `single_file` or `multiple_files`, which keep the media in the same bundle as every locale’s content file.
...
Source: https://sveltiacms.app/en/docs/i18n/structures

---

## Localized Slugs and Preview Paths

When i18n is enabled for an entry collection, [managing entry slugs](https://sveltiacms.app/en/docs/collections/entries/slugs#entry-slugs) (file names) across different locales can be challenging. Sveltia CMS offers two solutions: localized entry slugs and UUID-based slugs.
...
In a [nested collection](https://sveltiacms.app/en/docs/i18n/structures#nested-collections), the slug names the entry’s folder, so localizing the slugs also gives every locale its own [folder names](https://sveltiacms.app/en/docs/i18n/structures#localized-folder-names).

### Making Slugs Editable

Localized slugs can be edited for each locale in the [Slug panel](https://sveltiacms.app/en/docs/ui/content-editor#slug-panel) of the Content Editor, which has a section for each locale. When the slugs aren’t localized, a single slug is shared by every locale.

Instead of adding the `localize` filter to the template tags, you can localize the slugs with the `i18n` option of the [slug options object](https://sveltiacms.app/en/docs/collections/entries/slugs#making-slugs-editable). It accepts the following values:

- `true`: Each locale has a slug of its own. Every field tag in the template is filled with the locale’s own value, so `slug: { template: '{{title}}', i18n: true }` is equivalent to `slug: '{{title | localize}}'`.
- `duplicate` (default): The default locale’s slug is shared by every locale.

Like the `localize` filter, the `i18n` option only has an effect with the `multiple_files`, `multiple_folders` or `multiple_root_folders` structure, or a `folder` with the [`{{locale}}` placeholder](https://sveltiacms.app/en/docs/i18n/structures#custom-locale-folder-placement), where each locale has a file of its own.

To have users [type a slug](https://sveltiacms.app/en/docs/collections/entries/slugs#having-users-type-the-slug) for each locale, combine it with the `editable` option, without a `template`:
...
The special `{{fields._slug | localize}}` slug template tag has been deprecated in favour of the configuration above, and will be removed in Sveltia CMS v1.0.0.
...
This can be achieved by setting the `slug` option with the `{{uuid}}`, `{{uuid_short}}` or `{{uuid_shorter}}` [template tag](https://sveltiacms.app/en/docs/collections/entries/slugs#slug-template-tags) in your collection configuration:
...
Source: https://sveltiacms.app/en/docs/i18n/slugs
...
- Large Language Models (LLMs)
  ...
  - DeepSeek: [DeepSeek-V4.1-Flash](https://api-docs.deepseek.com/news/news260910)
