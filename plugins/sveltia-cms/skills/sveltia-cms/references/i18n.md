...
- Moving an entry with the [path editor](https://sveltiacms.app/en/docs/collections/entries#choosing-a-parent-folder) moves every locale’s file in the same commit. In the `subfolders` mode it takes the entry’s child entries along, in every locale.
...
This requires the `multiple_files` or `multiple_folders` structure, like localized slugs in general. With `subfolders: false`, entries are files rather than folders, so a folder is only localized when it holds an entry named with the `meta.path.index_file` option; any other folder keeps the same name in every locale.
